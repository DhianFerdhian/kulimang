...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$aplikasi) {
        $_SESSION['error'] = "Akses ditolak. Anda tidak memiliki izin untuk pekerjaan ini.";
    ...
    if (isset($_FILES['bukti_foto']) && $_FILES['bukti_foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['bukti_foto']['tmp_name'], $filePath)) {
            // Update database - PERBAIKAN: tanpa kolom catatan_selesai
            try {
                ...
                // Update tabel aplikasi_pekerjaan dengan status selesai
                // TANPA catatan_selesai karena kolom tidak ada
                $stmt = $pdo->prepare("UPDATE aplikasi_pekerjaan SET 
                                      status = 'selesai',
                                      updated_at = NOW()
                                      WHERE id = ?");
                $stmt->execute([$aplikasi_id]);
                
                $pdo->commit();
                
                $_SESSION['success'] = "Bukti pekerjaan berhasil diupload! Status pekerjaan telah berubah menjadi selesai.";
                $_SESSION['last_uploaded'] = $pekerjaan_id;
